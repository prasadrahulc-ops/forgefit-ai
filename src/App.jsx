...
// Storage abstraction — uses localStorage for the deployed app
const store = {
...
const S = {
  ...
  tab: (a) => ({ flex:1, padding:"10px", fontSize:12, fontWeight:700, border:"none", cursor:"pointer", borderRadius:8, background: a ? "rgba(198,241,53,0.12)" : "transparent", color: a ? accent : "rgba(240,240,240,0.4)", letterSpacing:1, textTransform:"uppercase", transition:"all 0.15s" }),
  ...
  unitBtn: (a) => ({ flex:1, padding:"10px", fontSize:12, fontWeight:700, border:"none", cursor:"pointer", background: a ? accent : "transparent", color: a ? "#0d0d0d" : "rgba(240,240,240,0.4)", transition:"all 0.15s" }),
  ...
  savedBtn: { width:"100%", background:"rgba(198,241,53,0.05)", border:"1px solid rgba(198,241,53,0.3)", color:"rgba(198,241,53,0.5)", borderRadius:10, padding:"14px", fontSize:13, fontWeight:700, cursor:"default", marginBottom:12 },
  restartBtn: { marginTop:16, background:"transparent", border:"1px solid rgba(255,255,255,0.1)", color:"rgba(240,240,240,0.4)", borderRadius:10, padding:"13px", width:"100%", fontSize:12, fontWeight:700, letterSpacing:1, textTransform:"uppercase", cursor:"pointer" },
  ...
  exGrid: { display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(130px, 1fr))", gap:10, marginTop:14 },
  exCard: { background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:10, overflow:"hidden", textAlign:"center" },
  exImg: { width:"100%", height:100, objectFit:"cover", display:"block" },
  exImgPlaceholder: { width:"100%", height:100, background:"rgba(255,255,255,0.05)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:24 },
  exName: { fontSize:11, padding:"6px 8px", color:"rgba(240,240,240,0.6)", lineHeight:1.3 },
};
...
const dietOpts = [
  ...
  { val:"indian vegetarian", label:"Indian Vegetarian", emoji:"🫙" },
  { val:"jain diet", label:"Jain Diet (no root veg)", emoji:"🕉️" },
...
const foodCultureOpts = [
  ...
  { val:"North Indian (Punjab, Delhi, UP cuisine)", label:"North Indian", emoji:"🫓" },
  { val:"South Indian (Tamil, Kerala, Karnataka)", label:"South Indian", emoji:"🥥" },
  ...
  { val:"Middle Eastern / Mediterranean", label:"Middle Eastern / Med", emoji:"🧆" },
...
function SingleOpt({ options, value, onChange, cols=1 }) {
  return (
    <div style={{ display:"grid", gridTemplateColumns: cols===2 ? "1fr 1fr" : "1fr", gap:8 }}>
      {options.map(o => {
        const s = value===o.val;
        return <div key={o.val} style={S.opt(s)} onClick={() => onChange(o.val)}>
          <span style={S.optEmoji}>{o.emoji}</span>
          <span style={S.optLabel(s)}>{o.label}</span>
          <span style={S.optCheck(s,false)}>{s?"✓":""}</span>
        </div>;
      })}
    </div>
  );
...
function MultiOpt({ options, value, onChange, cols=1 }) {
  ...
  return (
    <div style={{ display:"grid", gridTemplateColumns: cols===2 ? "1fr 1fr" : "1fr", gap:8 }}>
      {options.map(o => {
        const s = value.includes(o.val);
        return <div key={o.val} style={S.opt(s)} onClick={() => toggle(o.val)}>
          <span style={S.optEmoji}>{o.emoji}</span>
          <span style={S.optLabel(s)}>{o.label}</span>
          <span style={S.optCheck(s,true)}>{s?"✓":""}</span>
        </div>;
      })}
    </div>
  );
...
function parseExerciseList(text) {
  const section = parseSection(text, "EXERCISE LIST");
  if (!section) return [];
...
export default function App() {
  ...
  const msgs = ["Analysing your profile…","Designing your workout split…","Calibrating nutrition targets…","Selecting your supplement stack…","Finalising your plan…"];
  ...
  async function fetchExerciseImages(exercises) {
    const imgs = {};
    await Promise.all(exercises.map(async name => {
      try {
        const r = await fetch(`https://exercisedb-api.vercel.app/api/v1/exercises/name/${encodeURIComponent(name)}?limit=1`);
        const d = await r.json();
        const gif = d?.data?.[0]?.gifUrl || d?.exercises?.[0]?.gifUrl || d?.[0]?.gifUrl;
        if (gif) imgs[name] = gif;
      } catch {}
    }));
...
IMPORTANT: Address ALL goals simultaneously. Use ${form.foodCulture} cuisine for all meal examples.
...
### NUTRITION GUIDE
Daily calorie target, macros in grams. 3 sample meals using ${form.foodCulture} cuisine matching ${form.diets.join(", ")} preferences.
...
5 numbered actions for ${form.name} to take this week.

### EXERCISE LIST
List ONLY the exercise names from the workout plan above, one per line, no sets/reps/descriptions. Just the name. Maximum 9 exercises. Example:
Barbell Squat
Push-up
Dumbbell Row`;
    ...
    try {
      ...
      if (exercises.length) fetchExerciseImages(exercises);
    } catch(e) {
  ...
  function savePlan() {
    ...
    store.set("forgefit:plans", updated);
    setPlanSaved(true);
  }
  ...
  function restart() {
    setForm(emptyForm); setResult(null); setPlanSaved(false); setExerciseImages({}); setStep(0);
  }
  ...
  const showResults = (!loading && step===4 && displayResult) || viewingPlan;
      ...
      <div style={S.inner}>
        ...
        {!showResults && !loading && (
          <div style={S.tabs}>
            ...
            <button style={S.tab(activeTab==="saved")} onClick={()=>{ setActiveTab("saved"); setViewingPlan(null); }}>
        ...
        {loading && (
          <div style={S.loadWrap}>
            ...
            <div style={{ display:"flex", justifyContent:"center", gap:6 }}>
              {[0,1,2,3,4].map(i=><div key={i} style={{ width:6, height:6, borderRadius:"50%", background: i===loadMsg%5 ? accent : "rgba(255,255,255,0.12)", transition:"background 0.3s" }} />)}
            </div>
          </div>
        )}
          ...
          <>
            ...
            {savedPlans.length===0 ? (
              <div style={S.emptyState}>
                ...
                <p style={{ fontSize:14 }}>No saved plans yet</p>
                <p style={{ fontSize:12, marginTop:6 }}>Generate a plan and save it to access it here anytime</p>
              </div>
            ) : savedPlans.map(p => (
              <div key={p.id} style={S.planCard} onClick={()=>setViewingPlan(p)}>
                <div style={S.planCardTop}>
                  <div>
                    <div style={S.planCardName}>{p.name}'s Plan</div>
                    <div style={S.planCardDate}>{p.date}</div>
                  </div>
                  <button style={S.delBtn} onClick={e=>deletePlan(p.id,e)}>✕</button>
                </div>
                <div style={S.tagRow}>
                  {p.goals.map(g=><span key={g} style={S.tag}>{g}</span>)}
                  <span style={S.tagMuted}>{p.foodCulture?.split("(")[0].trim()}</span>
                </div>
              </div>
            ))}
          </>
        ...
        {!loading && activeTab==="new" && step===0 && (
          <>
            <p style={S.eyebrow}>Step 1 of 4</p>
            <h1 style={S.heading}>Who are you?</h1>
            <p style={S.sub}>Let's personalise everything for you</p>
            <div style={S.fg}>
              <div><label style={S.label}>First name</label><input style={S.input} placeholder="e.g. Arjun" value={form.name} onChange={e=>set("name",e.target.value)} /></div>
              <div><label style={S.label}>Age</label><input style={S.input} type="number" placeholder="e.g. 27" value={form.age} onChange={e=>set("age",e.target.value)} /></div>
              <div><label style={S.label}>Gender</label><SingleOpt options={genderOpts} value={form.gender} onChange={v=>set("gender",v)} /></div>
            </div>
            <button style={S.btn(!canNext[0])} disabled={!canNext[0]} onClick={()=>setStep(1)}>Continue →</button>
          </>
        )}

        {!loading && activeTab==="new" && step===1 && (
          <>
            ...
            <div style={S.fg}>
              <div>
                ...
                {form.heightUnit==="cm" ? (
                  <input style={S.input} type="number" placeholder="e.g. 175" value={form.heightCm} onChange={e=>set("heightCm",e.target.value)} />
                ) : (
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                    <input style={S.input} type="number" placeholder="Feet e.g. 5" value={form.heightFt} onChange={e=>set("heightFt",e.target.value)} />
                    <input style={S.input} type="number" placeholder="Inches e.g. 9" value={form.heightIn} onChange={e=>set("heightIn",e.target.value)} />
                  </div>
                )}
                {heightCmVal>50 && form.heightUnit==="ft" && <p style={S.hint}>≈ {heightCmVal} cm</p>}
              </div>
              <div>
                ...
                <input style={S.input} type="number" placeholder={form.weightUnit==="kg"?"e.g. 78":"e.g. 172"} value={form.weightVal} onChange={e=>set("weightVal",e.target.value)} />
                {weightKgVal>20 && form.weightUnit==="lbs" && <p style={S.hint}>≈ {weightKgVal} kg</p>}
              </div>
            </div>
            <button style={S.btnOut} onClick={()=>setStep(0)}>← Back</button>
            <button style={S.btn(!canNext[1])} disabled={!canNext[1]} onClick={()=>setStep(2)}>Continue →</button>
          </>
        )}

        {!loading && activeTab==="new" && step===2 && (
          <>
            ...
            <div style={S.fg}>
              <div>
                <label style={S.label}>Goals <span style={{ color:"rgba(198,241,53,0.5)", fontWeight:400, letterSpacing:0, textTransform:"none" }}>— pick one or more</span></label>
                <MultiOpt options={goalOpts} value={form.goals} onChange={v=>set("goals",v)} />
                {form.goals.length>0 && <div style={S.tagRow}>{form.goals.map(g=><span key={g} style={S.tag}>{g}</span>)}</div>}
              </div>
              <div><label style={S.label}>Experience level</label><SingleOpt options={levelOpts} value={form.level} onChange={v=>set("level",v)} /></div>
            </div>
            <button style={S.btnOut} onClick={()=>setStep(1)}>← Back</button>
            <button style={S.btn(!canNext[2])} disabled={!canNext[2]} onClick={()=>setStep(3)}>Continue →</button>
          </>
        )}

        {!loading && activeTab==="new" && step===3 && (
          <>
            <p style={S.eyebrow}>Step 4 of 4</p>
            <h1 style={S.heading}>Your lifestyle</h1>
            <p style={S.sub}>Your schedule, diet, and food culture</p>
            <div style={S.fg}>
              <div><label style={S.label}>Equipment access</label><SingleOpt options={equipOpts} value={form.equipment} onChange={v=>set("equipment",v)} /></div>
              <div>
                <label style={S.label}>Dietary preferences <span style={{ color:"rgba(198,241,53,0.5)", fontWeight:400, letterSpacing:0, textTransform:"none" }}>— pick all that apply</span></label>
                <MultiOpt options={dietOpts} value={form.diets} onChange={v=>set("diets",v)} cols={2} />
                {form.diets.length>0 && <div style={S.tagRow}>{form.diets.map(d=><span key={d} style={S.tag}>{d}</span>)}</div>}
              </div>
              <div>
                <label style={S.label}>Food culture / regional cuisine</label>
                <p style={{ fontSize:12, color:"rgba(240,240,240,0.35)", marginBottom:10 }}>All meal suggestions will use ingredients from your cuisine</p>
                <SingleOpt options={foodCultureOpts} value={form.foodCulture} onChange={v=>set("foodCulture",v)} cols={2} />
              </div>
              <div><label style={S.label}>Training days per week</label><SingleOpt options={dayOpts} value={form.days} onChange={v=>set("days",v)} cols={2} /></div>
            </div>
            <button style={S.btnOut} onClick={()=>setStep(2)}>← Back</button>
            <button style={S.btn(!canNext[3])} disabled={!canNext[3]} onClick={generate}>⚡ Generate My Plan</button>
          </>
        )}
        ...
        {showResults && (
          <>
            ...
            <div style={S.tagRow}>
              {displayForm.goals?.map(g=><span key={g} style={S.tag}>{g}</span>)}
              <span style={S.tagMuted}>{displayForm.days} days/week</span>
              <span style={S.tagMuted}>{displayForm.foodCulture?.split("(")[0].trim()}</span>
            </div>
                  ...
                  {s.key==="WORKOUT PLAN" && (() => {
                    ...
                    return (
                      <div style={{ marginTop:16, paddingTop:16, borderTop:"1px solid rgba(255,255,255,0.06)" }}>
                        <div style={{ fontSize:11, letterSpacing:2, textTransform:"uppercase", color:"rgba(198,241,53,0.6)", fontWeight:700, marginBottom:10 }}>Exercise Reference</div>
                        <div style={S.exGrid}>
                          {exercises.map(name => (
                            <div key={name} style={S.exCard}>
                              {displayImages[name]
                                ? <img src={displayImages[name]} alt={name} style={S.exImg} loading="lazy" onError={e=>{e.target.style.display="none";e.target.nextSibling.style.display="flex";}} />
                                : null}
                              <div style={{ ...S.exImgPlaceholder, display: displayImages[name] ? "none" : "flex" }}>🏋️</div>
            ...
            {!viewingPlan && (planSaved
              ? <div style={S.savedBtn}>✓ Plan saved to your library</div>
              : <button style={S.saveBtn} onClick={savePlan}>📋 Save this plan</button>
            )}
            <p style={S.disc}>AI-generated for educational purposes only. Consult a qualified fitness professional before starting any new exercise or supplement programme.</p>
            {!viewingPlan && <button style={S.restartBtn} onClick={restart}>↺ Generate a new plan</button>}
          </>
        )}
      </div>
    </div>
